...
def main_code():
	...
	for dataset in DATASETS:
		...
		for k in range(3, 25, 1):
			...
			table.rows = copy.deepcopy(original_rows)
			top_ranked_attributes = dict(sorted(entropyDict.items(), key=operator.itemgetter(1))[:k])
			...
			r = sum(RECALL[learner][1:]) / len(RECALL[learner][1:])
			fa = sum(FALSE_ALARM[learner][1:]) / len(FALSE_ALARM[learner][1:])
