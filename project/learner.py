...
def MiniBatch(train, test):
    ...
    # pairwise_distances_argmin(dtrain, k_means_cluster_centers)
    k_means_labels = mb.labels_
    ...
    for i, p in enumerate(predicted_clusters):
        ...
        predicted_row = pairwise_distances_argmin(
            dtest[i], train_elements_in_clusters).tolist()
        predicted_labels.append(
            train[dtrain.index(train_elements_in_clusters[predicted_row[0]])][-1])
...
def knnClassify(table, row, data):
    closest, furthest = table.closestAndFurthest(row, data)
    return closest[-1]

datasets = ['.csv', 'ivy-2.0.csv']
for dataset in datasets:
    ...
    for index in range(0, 25):
        ...
        train, test, train_label, test_label = train_test(
            table.rows, 5, index % 5)
        ...
        knn_predicted_labels = [knnClassify(table, x, train) for x in test]
        knn_time += (time.time() - start_time)
        recall_for_diff_algos['knn'].append(
            recall(knn_predicted_labels, test_label, 1))
        fAlarm_for_diff_algos['knn'].append(
            falseAlarm(knn_predicted_labels, test_label, 1))

        start_time = time.time()
        predicted_labels = MiniBatch(train, test)
        minibatch_time += (time.time() - start_time)
        recall_for_diff_algos['minibatch'].append(
            recall(predicted_labels, test_label, 1))
        fAlarm_for_diff_algos['minibatch'].append(
            falseAlarm(predicted_labels, test_label, 1))

        start_time = time.time()
        predicted_labels = kdTree(train, test)
        kdTree_time += (time.time() - start_time)
        recall_for_diff_algos['kdTree'].append(
            recall(predicted_labels, test_label, 1))
        fAlarm_for_diff_algos['kdTree'].append(
            falseAlarm(predicted_labels, test_label, 1))
